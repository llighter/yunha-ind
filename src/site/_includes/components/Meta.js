 ...
 */
 ...
 module.exports = (locale, page, collections) => {
     ...
     * Post authors should be albe to create a `social` object in their YAML
    ...
    function getMetaByPlatform(platform) {
        ...
        if (!thumbnail) {
            thumbnail = new URL(site.thumbnail, site.url);
        } else {
            thumbnail = new URL(path.join(pageUrl, thumbnail), site.url);
        }
    ...
    function renderFacebookMeta() {
        ...
        // probably find as those shouldn't show up in search results anyway.
